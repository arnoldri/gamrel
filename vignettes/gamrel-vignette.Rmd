...
Let $G(\cdot)$, $G_1(\cdot)$ and $G_2(\cdot)$ be non-normalised measures with support on the positive real line.  Then consider four cases:
  ...
  1. Increasing Failure Rate (IFR) (After @LoWeng:1989)
    \begin{equation}
       \lambda(t|G) = \int_0^t G({\rm d}u)
       (\#eq:ifr)
    \end{equation}
  1. Decreasing Failure Rate (DFR) (After @LoWeng:1989)
    \begin{equation}
       \lambda(t|G) = \int_t^\infty G({\rm d}u)
       (\#eq:dfr)
    \end{equation}
  1. Lo-Weng Bathtub (LWB) (After @LoWeng:1989)
    \begin{equation}
       \lambda(t|G) = \int_0^{|t-a|} G({\rm d}u)
       (\#eq:low-weng-bathtub)
    \end{equation}
    for constant $a>0$.
...
\begin{eqnarray}
   \lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      &=& \gamma \sum_{k=1}^K \tilde{w}_k I(\theta_k<t)\\
      &=& \gamma \tilde{\lambda}(t|\mathbf{\theta},\mathbf{v})\\
   \Lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      &=& \gamma \sum_{k=1}^K \tilde{w}_k \text{max}(0,t-\theta_k)\\
      &=& \gamma \tilde{\Lambda}(t|\mathbf{\theta},\mathbf{v})
\end{eqnarray}
...
\begin{eqnarray}
   T = \frac{-\log U + D^\ast_{k^\ast}}{C^\ast_{k^\ast}}
\end{eqnarray}
...
\begin{eqnarray}
   \lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      ...
      &=& \gamma \tilde{\lambda}(t|\mathbf{\theta},\mathbf{v})\\
   \Lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      &=& \gamma \sum_{k=1}^K \tilde{w}_k \text{min}(t,\theta_k)\\
      &=& \gamma \tilde{\Lambda}(t|\mathbf{\theta},\mathbf{v})
\end{eqnarray}
Following the same protocol as in the IFR case: 
if we define $\Lambda^\ast_k=\Lambda(\theta^\ast_k)$ with $\theta^\ast_0=0$ and $\Lambda^\ast_0=0$, then $\Lambda$ is piecewise linear between the points $\{(\theta^\ast_k,\Lambda^\ast_k)\}_{k=0}^K$.   If $U\sim\text{Uniform}(0,1)$ then $T$ is a draw from this distribution if
\begin{eqnarray}
   T = \frac{-\log U - D^\ast_{k^\ast}}{\gamma - C^\ast_{k^\ast}}
\end{eqnarray}
where $k^\ast = \text{max}\{k\in\{1,\ldots,K\}\,:\,\Lambda^\ast_{k}\leq -\log U\}$
...
\begin{eqnarray}
   \lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      &=& \gamma \sum_{k=1}^K \tilde{w}_k I(0<\theta_k<|t-a|)\\
      &=& \begin{cases}
   ...
   \Lambda(t|\gamma,\mathbf{\theta},\mathbf{v})
      &=& \begin{cases}
          \gamma\sum_{k=1}^K \tilde{w}_k I(\theta_k<a)\text{min}(t,a-\theta_k)\;\; & \text{if $t\leq a$}\\
...
and add a prior for $\lambda_0$:
\begin{equation}
    \lambda_0 \sim \text{Exp}(\nu)
\end{equation}
...
\begin{eqnarray}
   \lambda(t|\pi,\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1,\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)
   &=& \frac{f(t|\pi,\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1,\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)}{
             \bar{F}(t|\pi,\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1,\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)}\\
   &=& \frac{\pi \lambda_{\rm DFR}(t| \gamma_1,\mathbf{\theta}_1,\mathbf{v}_1) 
             \bar{F}_{\rm DFR}(t|\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1)
             +(1-\pi) \lambda_{\rm IFR}(t| \gamma_2,\mathbf{\theta}_2,\mathbf{v}_2) 
             \bar{F}_{\rm IFR}(t|\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)
   }{\pi \bar{F}_{\rm DFR}(t|\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1)
             +(1-\pi) \bar{F}_{\rm IFR}(t|\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)}\\
   \Lambda(t|\pi,\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1,\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)
   &=& -\log(\bar{F}(t|\pi,\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1,\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2))\\
   &=& -\log\left(\pi \bar{F}_{\rm DFR}(t|\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1)
             +(1-\pi) \bar{F}_{\rm IFR}(t|\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)\right)\\
   &=& -\log\left(\pi e^{-\Lambda_{\rm DFR}(t|\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1)}
             +(1-\pi) e^{-\Lambda_{\rm IFR}(t|\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)}\right)
\end{eqnarray}
Simulate as a mixture: draw $c\sim\text{Cat}(\{1,2\};(\pi,1-\pi))$ then if $c=1$ draw from the DFR component (parameters $\gamma_1,\mathbf{\theta}_1,\mathbf{v}_1)$), and if $c=2$ draw from the IFR component (parameters $\gamma_2,\mathbf{\theta}_2,\mathbf{v}_2)$).

### Log convex case

\begin{eqnarray}
   \frac{d\log\lambda(t|\gamma,\mathbf{\theta},\mathbf{v})}{dt} 
   &=& w_0 + \sum_{k=1}^K w_k I(0<\theta_k<t)\\
   \log\lambda(t|\gamma,\mathbf{\theta},\mathbf{v}) &=& \log\lambda_0 + w_0 t 
                                           + \sum_{k=1}^K w_k \text{max}(0,t-\theta_k)\\
   \lambda(t|\gamma,\mathbf{\theta},\mathbf{v}) &=& \lambda_0 \exp\left(w_0 t + \sum_{k=1}^K w_k \text{max}(0,t-\theta_k)\right)\\
\end{eqnarray}
Then, switching to the re-indexed set $\{(w^\ast_k,\theta^\ast_k)\}_{k=1}^K$
\begin{eqnarray}
   \Lambda(t|\gamma,\mathbf{\theta},\mathbf{v}) &=& \sum_{\ell=0}^{k^\ast(t)-1} 
                      \int_{\theta^\ast_\ell}^{\theta^\ast_{\ell+1}} 
                            \lambda(u|\gamma,\mathbf{\theta},\mathbf{v})\,{\rm d}u\\
                  & & + \int_{\theta^\ast_{k^\ast(t)}}^{t} \lambda(u|\gamma,\mathbf{\theta},\mathbf{v})\,{\rm d}u\\
