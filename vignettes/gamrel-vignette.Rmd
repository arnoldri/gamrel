...
\begin{eqnarray}
   w_k &=& \gamma v_k \prod{\ell < k} (1-v_\ell)\\
...
\begin{eqnarray}
  r &=& \frac{f(\theta^\ast_k|-)}{f(\theta_k|-)} 
    ...
    &=& LR(\Omega^+|\Omega) \times e^{-\phi(\theta^\ast_k-\theta_k)}\frac{\theta^\ast_k}{\theta_k}\\
  \log r &=& \log LR(\Omega^+|\Omega) -\phi(\theta^\ast_k-\theta_k) + \log\theta^\ast_k-\log\theta_k
\end{eqnarray}
...
\begin{eqnarray}
  r &=& \frac{f(v^\ast_k|-)}{f(v_k|-)} 
    ...
    &=& LR(\Omega^+|\Omega)      
         \left(\frac{v_k^\ast}{v_k}\right)^2\left(\frac{1-v_k^\ast}{1-v_k}\right)^{\alpha}\\
...
$\log w_k^\ast | \log w_k, \sigma_w^2 \sim N(\log w_k, \sigma_w^2)$
\begin{eqnarray}
  q(k, w^+_k | w_k, \gamma) 
...
\begin{eqnarray}
  ...
  v^+_\ell &=& \frac{w^+_\ell}{\sum_{j=\ell}^K w_j}
\end{eqnarray}
