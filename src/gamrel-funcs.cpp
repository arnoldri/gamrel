...
//' @export
// [[Rcpp::export]]
NumericVector lprior_circdr_c(double eta, 
                              double gamma,
                              NumericVector thetavec, 
                              NumericVector vvec,
                              double alpha, 
...
//' @export
// [[Rcpp::export]]
NumericVector lambda_func_cir_c(NumericVector tvec,
                                double lambda0, 
                                NumericVector thetavec, 
                                NumericVector wvec) {
   int n = tvec.size();
...
//' @export
// [[Rcpp::export]]
NumericVector lambda_func_cdr_c(NumericVector tvec,
                                double lambda0, 
                                NumericVector thetavec, 
                                NumericVector wvec) {
   int n = tvec.size();
...
//' @export
// [[Rcpp::export]]
NumericVector int_lambda_func_cir_c(NumericVector tvec, 
                                    double lambda0, 
                                    NumericVector thetavec, 
                                    NumericVector wvec) {
   int n = tvec.size();
   ...
   for(i=0; i<n; i++) {
     ...
     for(k=0; k<kmax; k++) {
       if(thetavec[k]<tvec[i]) {
         int_lambdavec[i] += 0.5*wvec[k]*pow(tvec[i]-thetavec[k],2); 
       }
     }
   }
...
//' @export
// [[Rcpp::export]]
NumericVector int_lambda_func_cdr_c(NumericVector tvec, 
                                    double lambda0, 
                                    NumericVector thetavec, 
                                    NumericVector wvec) {
   int n = tvec.size();
   ...
   esum = 0;
   for(k=0; k<kmax; k++) {
     esum += wvec[k]*pow(thetavec[k],2);
   }
   
   for(i=0; i<n; i++) {
     int_lambdavec[i] = lambda0*tvec[i] + 0.5*esum;
     for(k=0; k<kmax; k++) {
       if(thetavec[k]>tvec[i]) {
         int_lambdavec[i] += (-0.5)*wvec[k]*pow(thetavec[k]-tvec[i],2); 
       }
     }
   }
...
//' @export
// [[Rcpp::export]]
NumericMatrix both_lambda_func_cir_c(NumericVector tvec,
                                     double lambda0, 
                                     NumericVector thetavec, 
                                     NumericVector wvec) {
   int n = tvec.size();
   ...
   for(i=0; i<n; i++) {
     ...
     for(k=0; k<kmax; k++) {
       if(thetavec[k]<=tvec[i]) {
         bothlambdamat(i,0) += wvec[k]*(tvec[i]-thetavec[k]);
         bothlambdamat(i,1) += 0.5*wvec[k]*pow(tvec[i]-thetavec[k],2);  
       }
     }
   }
...
//' @export
// [[Rcpp::export]]
NumericMatrix both_lambda_func_cdr_c(NumericVector tvec,
                                     double lambda0, 
                                     NumericVector thetavec, 
                                     NumericVector wvec) {
   int n = tvec.size();
   ...
   for(i=0; i<n; i++) {
     bothlambdamat(i,0) = lambda0;
     bothlambdamat(i,1) = lambda0*tvec[i] + 0.5*esum;
     for(k=0; k<kmax; k++) {
       if(thetavec[k]>tvec[i]) {
         bothlambdamat(i,0) += wvec[k]*(thetavec[k]-tvec[i]);
         bothlambdamat(i,1) += (-0.5)*wvec[k]*pow(thetavec[k]-tvec[i],2);  
       }
     }
   }
